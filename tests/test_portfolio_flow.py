...
class PortfolioFlowTests(unittest.TestCase):
    ...
    def test_active_candidate_screening_rejects_excluded_returned_symbol(self):
        with patch(
            "src.strategy.portfolio_strategy.screen_active_tradfi_symbol",
            return_value={
                "metadata": {"screening_mode": "tradfi"},
            ...
            with self.assertRaises(portfolio_strategy.NoActiveCandidateError):
                portfolio_strategy._screen_active_candidate(
                    slot=_active_slot("active_1", "tradfi"),
                    config=_config(),
                    excluded_symbols=["esusdt"],
                )
    ...
    def test_four_active_slots_reserve_distinct_symbols_in_one_cycle(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir, patch(
            ...
            return_value=(
                "LONG",
                {"decision": {"decision": "LONG", "reason": "ZAI confirms the active setup."}},
                {
                    "ai_prompt_timeframe": "1h",
    ...
    def test_active_existing_position_between_18h_reviews_only_syncs_stop_loss(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir, patch(
            ...
            "src.strategy.portfolio_strategy._sync_fixed_stop_loss",
            return_value={"success": True, "changed": False, "stop_loss_pct": 0.05},
        ) as mocked_stop:
        ...
        self.assertEqual(result["trigger_reason"], "holding_until_rebalance")
    ...
    def test_active_existing_position_ignores_removed_price_trigger_distance(self):
        slot = _active_slot()
        slot_state = {
            "slot_id": "active_1",
            "kind": "active",
    ...
    def test_active_replacement_remembers_stale_symbol_as_previous(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir, patch(
            ...
            return_value=(
                "LONG",
                {"decision": {"decision": "LONG", "reason": "ZAI confirms the active candidate."}},
                {
                    "ai_prompt_timeframe": "1h",
                    "ai_prompt_candle_count": 3,
    ...
    def test_active_new_candidate_uses_zai_direction_after_screening(self):
        slot = _active_slot()
        slot_state = {
            "slot_id": "active_1",
            "kind": "active",
        ...
        with tempfile.TemporaryDirectory() as temp_dir, patch(
            ...
            return_value=(
                "LONG",
                {"decision": {"decision": "LONG", "reason": "ZAI sees stronger upside continuation."}},
                {
                    "ai_prompt_timeframe": "1h",
                    "ai_prompt_candle_count": 3,
        ...
        self.assertEqual(active_payload["decision_source"], "zai_trader")
    ...
    def test_active_due_rank_review_evaluates_current_top_symbol_with_zai(self):
        slot = _active_slot()
        slot_state = {
            "slot_id": "active_1",
            "kind": "active",
        ...
        with tempfile.TemporaryDirectory() as temp_dir, patch(
            ...
            return_value=(
                "LONG",
                {"decision": {"decision": "LONG", "reason": "ZAI keeps the current upside setup."}},
                {
                    "ai_prompt_timeframe": "1h",
                    "ai_prompt_candle_count": 168,
    ...
    def test_active_due_rank_review_excludes_other_open_positions_but_allows_current_symbol(self):
        slot = _active_slot()
        slot_state = {
            "slot_id": "active_1",
            "kind": "active",
        ...
        with tempfile.TemporaryDirectory() as temp_dir, patch(
            ...
            result, updated_state, active_symbol = portfolio_strategy._run_active_slot(
                ...
                api_secret="secret",
                account_overview={"equity": 1000.0, "available_balance": 500.0},
                open_positions=[_long_position("ESUSDT"), _short_position("NQUSDT")],
                reserved_symbols=set(),
                as_of_ms=(TEST_ACTIVE_RESCREEN_INTERVAL_HOURS * 60 * 60 * 1000) + 1000,
                cycle_dir_factory=lambda: temp_dir,
                notification_callback=None,
        ...
        self.assertTrue(result["success"])
        self.assertEqual(result["action"], "active_rank_review_failed_position_kept")
        self.assertEqual(active_symbol, "ESUSDT")
        self.assertEqual(updated_state["symbol"], "ESUSDT")
        excluded_symbols = set(mocked_screen.call_args.kwargs["excluded_symbols"])
        self.assertNotIn("ESUSDT", excluded_symbols)
        self.assertIn("NQUSDT", excluded_symbols)
    ...
    def test_active_due_rank_review_feeds_fixed_directions_into_rebalancer_without_trader(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir, patch(
            ...
            "src.strategy.portfolio_strategy._evaluate_slot_direction",
            side_effect=AssertionError("active rebalance should not call zai_trader direction review"),
        ) as mocked_ai, patch(
        ...
        self.assertEqual(result["decision_source"], "zai_rebalancer")
